...
use App\NavGuest;
use App\NavGuestImage;
use Storage;
use App\Http\Requests\Cms\NavGuestRequest;
use App\Http\Requests\Cms\NavGuestImageRequest;

class NavGuestController extends Controller {

    protected $route = 'nav-guests';
    protected $uploadDirectory = 'nav-guests';
        ...
        $this->datatables = [
            'nav_guests_category' => [
                ...
                'columns' => [
                    ...
                    [
                        'selector' => 'nav_guests.name',
                        'id' => 'name',
                        'name' => trans(\Locales::getNamespace() . '/datatables.name'),
                        'search' => true,
                        'link' => [
                            'selector' => ['nav_guests.slug'],
                            'icon' => 'folder-open',
                            'route' => $this->route,
                            'routeParameter' => 'slug',
                        ],
            ...
            'nav_guests_page' => [
                ...
                'columns' => [
                    ...
                    [
                        ...
                        'search' => true,
                        'link' => [
                            'selector' => ['nav_guests.is_category'],
                            'rules' => [
                                0 => [
                                    'column' => 'is_category',
                                    'value' => 0,
                    ...
                    [
                        'selector' => 'nav_guests.order',
                        'id' => 'order',
                        'name' => trans(\Locales::getNamespace() . '/datatables.order'),
                        'class' => 'text-center',
                    ],
                    [
                        'selector' => 'nav_guests.is_active',
                        'id' => 'is_active',
            ...
            'nav_guests_images' => [
                ...
                'columns' => [
                    ...
                    [
                        'selector' => 'nav_guests_images.order',
                        'id' => 'order',
                        'name' => trans(\Locales::getNamespace() . '/datatables.order'),
                        'class' => 'text-center',
                    ],
                    [
                        'selector' => 'nav_guests_images.is_active',
                        'id' => 'is_active',
                        'name' => trans(\Locales::getNamespace() . '/datatables.status'),
                        'class' => 'text-center',
                        'status' => [
                    ...
                    [
                        'selector' => 'nav_guests_images.name',
                        'id' => 'name',
                        'name' => trans(\Locales::getNamespace() . '/datatables.name'),
                        'search' => true,
                    ],
                    [
                        'selector' => 'nav_guests_images.file',
                        'id' => 'file',
                        'name' => trans(\Locales::getNamespace() . '/datatables.image'),
                        'order' => false,
                        'class' => 'text-center',
                        'thumbnail' => [
                            'selector' => ['nav_guests_images.uuid', 'nav_guests_images.title'],
                            'title' => 'title',
                            'id' => 'uuid',
                        ],
                    ],
                    [
                        'selector' => 'nav_guests_images.size',
                        'id' => 'size',
    ...
    public function index(DataTable $datatable, NavGuest $page, Request $request, $slugs = null)
    {
        $uploadDirectory = $this->uploadDirectory;
        ...
        if ($slugs) {
            ...
            $pages = NavGuest::select('id', 'parent', 'slug', 'is_category', 'name', 'is_multi_page')->get()->toArray();
            ...
            if ($row = \Slug::arrayMatchSlugsRecursive($slugsArray, $pages)) { // match slugs against the pages array
                ...
                if ($row['is_category']) { // it's a category
                    ...
                    $datatable->setup($page, 'nav_guests_page', $this->datatables['nav_guests_page']);

                    if ($row['is_multi_page']) {
                        $datatable->setOption('expandDirectory', \Config::get('upload.sliderDirectory') . '/', 'nav_guests_images');
                        $datatable->setup(NavGuestImage::where('nav_guest_id', $row['id']), 'nav_guests_images', $this->datatables['nav_guests_images']);
                    ...
                    if ($parent && $parent['is_multi_page']) {
                        $datatable->setOption('buttons', ['upload' => ['single' => 'true']], 'nav_guests_images');
                        $datatable->setOption('expandDirectory', \Config::get('upload.pageDirectory') . '/', 'nav_guests_images');
                    } else {
                        $datatable->setOption('expandDirectory', \Config::get('upload.sliderDirectory') . '/', 'nav_guests_images');
                    }
            ...
            $datatable->setup($page, 'nav_guests_category', $this->datatables['nav_guests_category']);
        }
    ...
    public function create(Request $request)
    {
        ...
        $options = [];
        foreach (trans(\Locales::getNamespace() . '/multiselect.pageTypesGuests') as $key => $value) {
            array_push($options, [
                'id' => $key,
                'name' =>  $value,
            ]);
    ...
    public function store(DataTable $datatable, NavGuest $page, NavGuestRequest $request)
    {
        ...
        $newNav = NavGuest::create($request->all());
    ...
    public function destroy(DataTable $datatable, NavGuest $page, Request $request)
    {
        $count = count($request->input('id'));

        $directories = NavGuest::find($request->input('id'))->lists('slug');
    ...
    public function update(DataTable $datatable, NavGuest $nav, NavGuestRequest $request)
    {
        $page = NavGuest::findOrFail($request->input('id'))->first();
    ...
    public function upload(Request $request, NavGuest $nav, FineUploader $uploader, $chunk = null)
    {
        ...
        if (isset($response['success']) && $response['success'] && isset($response['fileName'])) {
            if ($reupload) {
                ...
                $image = NavGuestImage::findOrFail($request->input('row'));
                $status = $image->is_active;

                Storage::disk('local-public')->deleteDirectory($uploader->uploadDirectory . '/' . $image->uuid);
            } else {
                $status = 1;
                $image = new NavGuestImage;
                $image->nav_guest_id = $request->input('id');
                $image->order = NavGuestImage::where('nav_guest_id', $request->input('id'))->max('order') + 1;
            }
            ...
            $statusData = [];
            foreach ($this->datatables['nav_guests_images']['columns'] as $column) {
    ...
    public function destroyImage(DataTable $datatable, NavGuest $nav, NavGuestImage $image, Request $request)
    {
        $count = count($request->input('id'));

        $uuids = NavGuestImage::find($request->input('id'))->lists('nav_guest_id', 'uuid');
        ...
        if ($count > 0 && $image->destroy($request->input('id'))) {
            ...
            \DB::statement('SET @pos := 0');
            \DB::update('update ' . $image->getTable() . ' SET `order` = (SELECT @pos := @pos + 1) WHERE nav_guest_id = ? ORDER BY `order`', [$page]);
            ...
            $datatable->setup(NavGuestImage::where('nav_guest_id', $page), $request->input('table'), $this->datatables[$request->input('table')], true);
            $datatable->setOption('url', \Locales::route($this->route, implode('/', $slugs)));
            $datatables = $datatable->getTables();
    ...
    {
        $image = NavGuestImage::findOrFail($id);
    ...
    public function updateImage(DataTable $datatable, NavGuest $nav, NavGuestImage $navImage, NavGuestImageRequest $request)
    {
        $image = NavGuestImage::findOrFail($request->input('id'))->first();

        $order = $request->input('order');
        if (!$order || $order < 0) {
            $order = $image->order;
        } elseif ($order) {
            $navImage = $navImage->where('nav_guest_id', $image->nav_guest_id);
        ...
        if ($image->update($request->all())) {
            ...
            $parentModel = $nav->select('join.is_multi_page')->leftJoin($nav->getTable() . ' as join', $nav->getTable() . '.parent', '=', 'join.id')->find($image->nav_guest_id);

            $image = $image->where('nav_guest_id', $image->nav_guest_id);
    ...
    public function changeStatus($id, $status)
    {
        ...
        $href = '';
        $img = '';
        foreach ($this->datatables['nav_guests_page']['columns'] as $column) {
            if ($column['id'] == 'is_active') {
